...
class GenerationsService extends ApiService {
  ...
  async getGenerationDetails(
    ...
    try {
      ...
      console.error(
        `Error fetching details for generation ${idOrName}:`,
        error,
      );
      return null;
    }
   ...
   */
  async getGenerationWithDetails(
    idOrName: string | number,
  ): Promise<IGenerationDetail | null> {
    if (!idOrName) return null;
    ...
    try {
          ...
          const movesWithDetails = await Promise.all(
            generation.moves.map(async (move) => {
              try {
                // Extract ID from URL
                const id = parseInt(
                  move.url.split("/").filter(Boolean).pop() || "0",
                );
      ...
      console.error(
        `Error fetching enhanced details for generation ${idOrName}:`,
        error,
      );
      return null;
    }
