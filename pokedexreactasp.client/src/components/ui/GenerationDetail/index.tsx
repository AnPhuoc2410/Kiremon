...
const GenerationDetail: React.FC<GenerationDetailProps> = ({ generation }) => {
  ...
  const totalPokemonPages = Math.ceil(
    generation.pokemon_species.length / ITEMS_PER_PAGE,
  );
  ...
  return (
    <GenerationContainer theme={regionTheme}>
      <TitleArea theme={regionTheme}>
        ...
        <Text as="p">Region: {formatName(generation.main_region.name)}</Text>
      </TitleArea>
                ...
                <Link to={`/pokemon/${species.name}`} key={species.name}>
                  <PokemonCard theme={regionTheme}>
                    <LazyLoadImage
                      src={
                        species.sprite ||
                        `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${extractIdFromUrl(species.url)}.png`
                      }
                      alt={species.name}
                      effect="opacity"
                      width={96}
