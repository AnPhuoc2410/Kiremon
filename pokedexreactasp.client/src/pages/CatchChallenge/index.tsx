...
const CatchChallenge: React.FC = () => {
  ...
  const [best, setBest] = useState<number>(() => Number(localStorage.getItem('pokegames@catchBest') || 0));
  ...
  const questionTime = difficulty === 'easy' ? 20 : difficulty === 'hard' ? 12 : 16; // seconds
  ...
  useEffect(() => {
    ...
    const id = setTimeout(() => { setTimeLeft(t => t - 1); sfx.tick(); }, 1000);
  ...
  const tryCatch = () => {
    ...
    const base = difficulty === 'easy' ? 0.45 : difficulty === 'hard' ? 0.25 : 0.35;
        ...
        {loading || !pokemon ? (
          ...
          <>
            ...
            <ThrowArea>
              ...
              <Button variant="dark" onClick={() => { setScore(0); setThrows(5); setStreak(0); setTimeLeft(questionTime); }}>Reset</Button>
            </ThrowArea>
          </>
        )}
      </GameCard>
