...
namespace PokedexReactASP.Infrastructure.Services
{
    public class WildAreaService : IWildAreaService
    {
        ...
        private async Task<WildAreaSettings> GetSettingsAsync()
        {
            if (_memoryCache.TryGetValue("WildAreaSettings", out WildAreaSettings? cachedSettings) && cachedSettings != null)
            {
            ...
            _memoryCache.Set("WildAreaSettings", settings, TimeSpan.FromMinutes(5));

            return settings;
        }

        public async Task<IReadOnlyList<WildAreaOptionDto>> GetAvailableAreasAsync()
        {
            var settings = await GetSettingsAsync();
            var areas = GetConfiguredAreas(settings)
        ...
        public async Task<WildAreaDto> GetCurrentWildAreaAsync(string userId, string? areaCode = null)
        {
            ...
            if (spawns.Count == 0)
            {
                spawns = await GenerateSpawnsAsync(userId, area, now, settings);
            }
        ...
        public async Task<WildAreaDto> RefreshWildAreaAsync(string userId, string? areaCode = null)
        {
            ...
            var newSpawns = await GenerateSpawnsAsync(userId, area, now, settings);
        ...
        private async Task<List<WildAreaSpawn>> GenerateSpawnsAsync(string userId, WildAreaConfig area, DateTime now, WildAreaSettings settings)
        {
            var interval = area.ResolveResetIntervalMinutes(settings);
            var spawnCount = area.ResolveSpawnCount(settings);
            var maxAttempts = Math.Max(1, settings.MaxAttemptsPerSpawn);
            var expiresAt = now.AddMinutes(interval);
            var weights = area.BuildWeights(settings.BuildWeights());
            ...
            for (var i = 0; i < spawnCount; i++)
            {
                ...
                if (rarity == WildSpawnRarity.Legendary && !area.ResolveAllowLegendary(settings))
                {
                    rarity = WildSpawnRarity.Epic;
                }

                var selectedMetadata = await PickCandidateAsync(area, rarity, settings);
        ...
        private async Task<PokemonSpawnMetadata> PickCandidateAsync(WildAreaConfig area, WildSpawnRarity rolledRarity, WildAreaSettings settings)
        {
            var candidates = await _candidateService.GetCandidatesWithFallbackAsync(area, settings, rolledRarity);
        ...
        private List<WildAreaConfig> GetConfiguredAreas(WildAreaSettings settings)
        {
