...
export const StatCard = styled.div<{ isComplete?: boolean }>`
  background: ${(props) => (props.isComplete ? colors["sky-500"] : "white")};
  border: 2px solid
    ${(props) => (props.isComplete ? colors["blue-200"] : colors["gray-200"])};
  border-radius: 12px;
...
export const StatPill = styled.button<{
  ...
  opacity: ${(props) => (props.isDisabled && !props.isSelected ? 0.3 : 1)};
  transition: all 0.2s;
...
export const ScoreValue = styled.div<{ highlight?: boolean }>`
  ...
  font-weight: 800;
  color: ${(props) =>
    props.highlight ? colors["green-500"] : colors["blue-600"]};
`;
